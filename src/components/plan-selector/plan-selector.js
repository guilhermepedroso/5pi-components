...
const partnerConfig = {
  "parceiros/somos": {
    ...
    mentoria: {
      label: "Mentoria Trader",
      url: "https://web.whatsapp.com/send/?phone=5511976044688&text=Ol%C3%A1%2C%20gostaria%20de%20contratar%20a%20Mentoria%20Trader%20do%20escrit%C3%B3rio%20Somos",
    },
  },
  "parceiros/bs": {
    overrides: educacionalOverrides,
    mentoria: {
      label: "Mentoria Trader",
      url: "https://web.whatsapp.com/send/?phone=5511976044688&text=Ol%C3%A1%2C%20gostaria%20de%20contratar%20a%20Mentoria%20Trader%20do%20escrit%C3%B3rio%20Barsi",
    },
  },
};
...
function getPartnerConfig() {
  const path = window.location.pathname;
  for (const [key, config] of Object.entries(partnerConfig)) {
...
function getOverridesForPath() {
  return getPartnerConfig()?.overrides ?? [];
}
...
const buildOffersViewModel = () => {
  ...
  return {
    ...
    isPartner: !!getPartnerConfig(),
    mentoria: getPartnerConfig()?.mentoria ?? null,
    partnerSlug: (() => {
      const m = window.location.pathname.match(/\/parceiros\/([^/]+)/);
      return m ? m[1] : null;
    })(),
  };
};
